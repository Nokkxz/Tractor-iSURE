...
#define SET_SPEED 20.0f
#define SET_TORQUE 150.0f

#define STEER_INTERVAL 0.01
#define MAX_STEER_ANGLE 0.6

double emmiter_send_buf[10];

bool is_record = true; // record or not
FILE *file_sin = NULL;
//char* file_path = "C:\\Users\\terrancecameron\\Documents\\ND-Project-Webots\\controllers\\sinusodal_test\\sin_data_sand.txt"; // file path
char* file_path = "sin_data_sand.txt"; // file path
int max_tick = 2000;
...
void print_help() {
  printf("SINUSOIDAL TEST -- You can drive this vehicle IF YOU WANT TO MESS UP THE TEST\n");
...
void steer_by_angle(double angle)
{
  wb_motor_set_position(left_steer, angle);
  wb_motor_set_position(right_steer, angle);
}
...
void sinusoidal_test(int tick) {

  last_angle = angle;
  angle = MAX_STEER_ANGLE * sin((double)tick*STEER_INTERVAL);
  steer_by_angle(angle);
  ...
  if(last_angle<0 && angle>0) { // check if is a new period
    period_num++;
  }
  
  if(start_record) {
    fprintf(file_sin, "%f\t%f\n", coords[X], coords[Z]); // input x and z into the file
    printf("recording...\n");
  }

  if(is_record && period_num>0){
    start_record = true;
  }
  
  if(start_record && period_num>3){
    start_record = false;
    fclose(file_sin); // close the file after 3 period
  }
}
...
int main(int argc, char **argv) {
  ...
  while (wb_robot_step(time_step) != -1) {
    // printf("tick: %d \n", tick);
    tick++;
    if(tick>200 && start==false){ // Wait for the tractor to get the set speed
      start = true;
      tick = 0;
    }
    if(start==true){
      sinusoidal_test(tick);
    }
